...
- **Identity (Microsoft Entra External ID)** — external tenant; the web app registers as a relying application (OIDC). User roles (Admin/Teacher/Approver/Parent/Student/Safeguarding Officer) are modelled as application-level claims/roles, not native Entra External ID groups, given the small scale and need for domain-specific rules (e.g. a Parent's role is scoped to specific Student records, not just a flat role).

## Decided

- **Scheduled jobs:** Azure App Service WebJobs — co-located with the web app, no separate Azure resource, matching the solo-maintainer preference for fewest moving parts (`raid.md` R-002).
