...
| BR-003 Safeguarding & consent | Annual consent, DBS, expiry blocking, guardian-link verification (`functional.md`) | `data-design.md` (ConsentRecord, ConsentSensitiveDetails, DbsCheck, StudentGuardianLink), `low-level-design.md` (ConsentExpiryJob) | TC-002, TC-009, TC-012, TC-013, TC-014 | Designed |
...
| NFR-001 Access control / PII | Role-based + resource-based authorization, DBS/consent-sensitive-detail whole-entity restriction, read audit log (`non-functional.md`, `governance/security-compliance.md`) | `low-level-design.md` (authorization model), `adr/0002`, `adr/0004` | TC-006, TC-007, TC-008, TC-011, TC-014 | Designed |
...
| NFR-004 Compatibility | Named browser/OS/PWA target (`non-functional.md`) | `adr/0001-technology-stack.md` (Blazor) | — | Accepted, not test-gated at design gate |
