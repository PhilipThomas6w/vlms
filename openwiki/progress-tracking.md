...
- **Precondition — flagged, not silently patched over:** this service expects an *open* `StudentRankProgress` row (`CompletedAt == null`) to already exist for the student's current rank before it's ever called, and throws a clear `InvalidOperationException` if none is found rather than fabricating one. Nothing in this codebase yet creates that first row — the student-registration/enrolment flow that will (STATE.md Next item 1, "Guardian-link creation flow ... at student registration") hasn't been built. Tests seed the initial row directly.
