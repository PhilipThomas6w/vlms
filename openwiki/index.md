...
Four `STATE.md` items are Done: the EF Core data model + ADR-0004 access control, Entra sign-in + provisioning + authorization, curriculum management (`LessonProposalService` + Teacher/Approver pages, plus the interactive-render-mode auth-resolution fix it depended on), and progress tracking (`CompletionService`/`PromotionService`/`CertificateService`, service-layer only — no UI yet). `Vlms.Web/Components/Pages/` has real VLMS UI (`Home.razor`, `Curriculum/TeacherProposals.razor`, `Curriculum/ApproverProposals.razor`) alongside the template's `Error.razor`/`NotFound.razor`; progress tracking has no consuming page yet, so `AzureBlobStorage`/`CompletionService`/`PromotionService`/`CertificateService` are not wired into `Program.cs`. See `STATE.md` for the live queue.
